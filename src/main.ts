...
function makeLobby() {
  ...
  queueMicrotask(() => {
    ...
    const fillMaps = () => {
      mapSel.innerHTML = "";
      const pool = mapsForMode(state.room.mode);
      const mapList = pool.length ? pool : MAPS;
      ...
      if (!mapList.find((m) => m.id === state.room.mapId) && mapList[0]) {
        state.room.mapId = mapList[0].id;
      }
...
function mountGame(stage: HTMLElement, canvas: HTMLCanvasElement) {
  ...
  hud.innerHTML = `
    ...
    <div class="controls-hint">Arrows/WASD thrust · X heavy · Z special · Esc menu${online ? ` · ${isHost ? "HOST" : "CLIENT"} ${state.roomCode ?? ""}` : ""}</div>
