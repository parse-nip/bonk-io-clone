...
- **Player controls are continuous Matter thrusters** (Left/Right/Up/Down `applyForce` every frame, full air control), matching HTML5 bonk’s Box2D disc model — **not** the OSU `bonk_v6` kinematic sketch. Real client gravity is `(0, 20)`; we map that feel via Matter `gravity.scale: 0.001` + map `gravity`. Thruster force is **~78% of weight** so holding Up cannot fly (you bounce). Disc-like restitution (~0.95), heavy ≈ **2× mass**. Holding a direction on narrow maps still rolls you off quickly — that is correct. Verify with `npm run test:movement` (thrusters, no-fly, fall-off, collision momentum, heavy).
...
- **Snapshots use Matter body velocities** (`body.velocity.x/y`). Do not reintroduce kinematic tutorial integration for online play — it created an invisible full-width floor bounce and zeroed knockback.
