...
- **Player controls are continuous Box2D thrusters** (`applyForceToCenter` on Left/Right/Up/Down every frame, full air control), matching HTML5 bonk’s disc model — **not** the OSU `bonk_v6` kinematic sketch. Real client gravity is `(0, 20)` (confirmed in `js/alpha2s.js`). This clone uses tutorial-scale mass/radius with map gravity ≈ **420** and **split** thrusters (strong horizontal, weaker vertical ≈ **0.32×** weight) so Up cannot float/hover. Heavy ≈ **2× density/mass**. Verify with `npm run test:movement`.
