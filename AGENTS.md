...
- **Player controls are continuous thrusters, not jumps.** Arrow keys (P1) / WASD (P2) are bound on `window` in `src/game/input.ts` and apply force every frame in all 4 directions (`applyThrusterForce` in `src/game/engine.ts`), matching the OSU bonk physics tutorial (`Fnet = F_thrust`, gravity from Matter). There is no grounded jump impulse. Holding a direction on narrow maps still rolls you off quickly — that is expected, not a broken control path. Verify with `npm run test:movement`.
